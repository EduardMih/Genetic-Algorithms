...
def binarytoGray(binary):
    ...
    for i in range(1, len(binary)):
        gray.append(xor_c(binary[i - 1], binary[i]))
...
def hill_climbing(fct, improve_meth, current):
    t = 0
    best = current
...
def decode(bitstring):
    ...
    bitstring = graytoBinary(bitstring)
...
def GA(fct):
    ...
    while generation < 1000:
        ...
        if fct(decode(cand)) < fct(decode(best)):
            best = cand
            cand = hill_climbing(fct, first_improve, cand)
            best = cand
